...
def get_initializer(method, activation):
...
def build_mlp(
    ...
    *,
    ...
    bias_init="zeros",
    norm_type = None,
    add_input_activation: bool=False,
    add_input_norm: bool = False,
    add_output_activation: bool = False,
...
class MultimodalBertEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.visual_mlps = build_mlp(config.image_feat_size, hidden_dim=1024, 
                                        output_dim=config.hidden_size, hidden_depth=3, activation="tanh",
                                        norm_type="layernorm")
        self.image_position_embeddings = nn.Embedding(50, config.hidden_size)
        self.union_position_embeddings = nn.Embedding(50+config.max_position_embeddings, config.hidden_size)

    def forward(self, input_ids, target_feats, target_span, bert_emb):
        seq_length = input_ids.size(1)
        target_length = target_feats.size(1)

        text_position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        text_position_ids = text_position_ids.unsqueeze(0).expand_as(input_ids)
        
        image_position_ids = torch.arange(target_length, dtype=torch.long, device=input_ids.device)
        image_position_ids = image_position_ids.unsqueeze(0).expand(target_feats.size(0), -1)
        
        token_type_ids = torch.zeros((input_ids.size(0), seq_length+target_length), dtype=torch.long, device=input_ids.device)
        token_type_ids[:, input_ids.size(1):] = 1

        words_embeddings = bert_emb.word_embeddings(input_ids)
        target_embeddings = self.visual_mlps(target_feats)

        text_position_embeddings = bert_emb.position_embeddings(text_position_ids)
        image_position_embeddings = self.image_position_embeddings(image_position_ids)
        position_embeddings = torch.cat([text_position_embeddings, image_position_embeddings], dim=1) 

        token_type_embeddings = bert_emb.token_type_embeddings(token_type_ids)

        union_pos_embeddings = self.get_union_pos_embedding(input_ids, target_span, target_length)

        prompt_embeddings = torch.cat([words_embeddings, target_embeddings], dim=1)
        embeddings = prompt_embeddings + position_embeddings + token_type_embeddings + union_pos_embeddings

        embeddings = bert_emb.LayerNorm(embeddings)
        embeddings = bert_emb.dropout(embeddings)
        return embeddings

    def get_union_pos_embedding(self, input_ids, target_span, target_length):
        seq_length = input_ids.size(1)
        total_length = seq_length + target_length
        bs = input_ids.size(0)
        batch_position_ids = []
        for i in range(bs):
            position_ids = torch.arange(total_length, dtype=torch.long, device=input_ids.device)
            span = np.array(target_span[i])
            span += 1
            for j in range(seq_length):
                upper_num = np.sum(span < j)
                position_ids[j] = j + upper_num
            for j in range(len(span)):
                upper_num = np.sum(span < span[j])
                position_ids[seq_length+j] = span[j] + upper_num + 1
            batch_position_ids.append(position_ids)
        batch_position_ids = torch.stack(batch_position_ids, dim=0)
        union_position_embeddings = self.union_position_embeddings(position_ids)
        return union_position_embeddings

class MultimodalFusion_variant1(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.image_position_embeddings = nn.Embedding(50, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, txt_embs, target_feats, target_span):
        target_length = target_feats.size(1)

        image_position_ids = torch.arange(1, dtype=torch.long, device=txt_embs.device)
        image_position_ids = image_position_ids.unsqueeze(0).expand(target_feats.size(0), -1)
        image_position_embeddings = self.image_position_embeddings(image_position_ids)

        target_embs = target_feats + image_position_embeddings
        embeddings = torch.cat([txt_embs, target_embs], dim=1)

        union_pos_embeddings = self.get_union_pos_embedding(txt_embs, target_span, target_length)
        embeddings = embeddings + union_pos_embeddings
...
class MultimodalFusion_variant2(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.visual_mlps = build_mlp(config.image_feat_size, hidden_dim=1024, 
                                        output_dim=config.hidden_size, hidden_depth=3, activation="tanh",
    ...
    def forward(self, txt_embs, target_feats, target_span):
        ...
        target_embs = self.visual_mlps(target_feats)
        target_embs = target_feats + image_position_embeddings
        embeddings = torch.cat([txt_embs, target_embs], dim=1)
...
class MultimodalFusion_variant3(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.image_position_embeddings = nn.Embedding(50, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
    ...
    def forward(self, txt_embs, target_feats, target_span, extended_txt_masks):        
        target_length = target_feats.size(1)
        ...
        for layer_module in self.layer:
            temp_output = layer_module(embeddings, extended_txt_masks)
            embeddings = temp_output[0]
...
class MultimodalFusion_variant4(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.image_position_embeddings = nn.Embedding(50, config.hidden_size)
        self.LayerNorm = BertLayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        ...
        self.layer = nn.ModuleList(
            [BertXLayer(config) for _ in range(1)]
        )
    ...
    def forward(self, txt_embs, target_feats, target_span, extended_txt_masks):     
        ...
        for layer_module in self.layer:
            temp_output = layer_module(target_embs, txt_embs, extended_txt_masks)
            target_embs = temp_output[0]
...
class BertXLayer(nn.Module):
    ...
    def forward(self, input_tensor, ctx_tensor, attention_mask):
        attention_outputs = self.attention(input_tensor, ctx_tensor, attention_mask)
        attention_output = attention_outputs[0]
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        outputs = (layer_output,) + attention_outputs[1:]  # add attentions if we output them
...
class ImageEmbeddings(nn.Module):
    ...
    def forward(
        ...
        has_obj = traj_obj_img_fts is not None and traj_obj_img_fts.nelement() > 0
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.mm_embeddings = MultimodalBertEmbeddings(config)
        self.lang_encoder = LanguageEncoder(config)
        # self.fusion_layer = MultimodalFusion_variant1(config)
    ...
    def forward_text(self, txt_ids, txt_masks, target_fts, target_span):
        txt_embeds = self.mm_embeddings(txt_ids, target_fts, target_span, self.embeddings)
        txt_embeds = self.lang_encoder(txt_embeds, txt_masks)

        # # for variant 1 & 2
        # txt_token_type_ids = torch.zeros_like(txt_ids)
        # txt_ids_size = txt_ids.size(1)
        # original_txt_masks = txt_masks[:,:txt_ids_size]
        # txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
        # txt_embeds = self.lang_encoder(txt_embeds, original_txt_masks)
        # txt_embeds = self.fusion_layer(txt_embeds, target_fts, target_span)
        ...
        # txt_embeds = self.lang_encoder(txt_embeds, original_txt_masks)
        # extended_txt_masks = extend_neg_masks(txt_masks)
        # txt_embeds = self.fusion_layer(txt_embeds, target_fts, target_span, extended_txt_masks)
        ...
        # txt_embeds = self.embeddings(txt_ids, token_type_ids=txt_token_type_ids)
        # txt_embeds = self.lang_encoder(txt_embeds, original_txt_masks)
        # extended_txt_masks = extend_neg_masks(original_txt_masks)
        # txt_embeds = self.fusion_layer(txt_embeds, target_fts, target_span, extended_txt_masks)
    ...
    def forward_panorama_per_step(
        ...
        has_obj = obj_img_fts is not None and obj_img_fts.nelement() > 0
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            txt_embeds = self.forward_text(batch['txt_ids'], batch['txt_masks'], 
                                            batch['target_fts'], batch['target_span'])
            return txt_embeds
