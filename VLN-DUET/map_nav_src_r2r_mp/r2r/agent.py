...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def _prompt_variable(self, obs):
        ...
        # add target visual features
        target_fts = []
        target_span = []
        max_lm_len = max([len(ob['mp_pos']) for ob in obs])
        if max_lm_len == 0:
            target_fts = torch.zeros((len(obs), 1, self.args.image_feat_size)).cuda()
            target_mask = torch.zeros((len(obs), 1), dtype=torch.bool).cuda()
            target_span = [[]]*len(obs)
        else:
            target_mask = torch.zeros((len(obs), max_lm_len), dtype=torch.bool).cuda()
            with h5py.File("../datasets/R2R/features/gelr2r_obj_features.hdf5", 'r') as f:
                for idx, ob in enumerate(obs):
                    instr_id = ob['instr_id']
                    fts = np.zeros((max_lm_len, self.args.image_feat_size), dtype=np.float32)
                    for i_lm, lm in enumerate(ob['landmarks']):
                        lm_new = lm.replace("/", " or ")
                        obj_id = f"{i_lm}_{lm_new}"
                        key = f"{instr_id}_{obj_id}_full" if self.args.pano_lm else f"{instr_id}_{obj_id}"
                        ft = f[key][...].astype(np.float32)
                        fts[i_lm] = ft
                        if ob['mp_pos'][i_lm] == 1:
                            target_mask[idx, i_lm] = True
                    target_fts.append(fts)
                    target_span.append(ob['text_span'])

            target_fts = np.stack(target_fts, 0)
            target_fts = torch.from_numpy(target_fts).cuda()
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        # prompt input: txt_ids, txt_masks, target_fts
        prompt_inputs = self._prompt_variable(obs)
        txt_embeds = self.vln_bert('language', prompt_inputs)
        ...
        for t in range(self.args.max_action_len):
            ...
            nav_inputs.update({
                ...
                'txt_masks': prompt_inputs['txt_masks'],
            })
