...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _prompt_variable(self, obs):
        ...
        for idx, ob in enumerate(obs):
            if ob['mp']:
                instr_id = f"{ob['path_id']}_{ob['gt_obj_id']}"
                scan = ob['scan']
                pano = ob['original_env_vp']
                key = f"{scan}_{pano}_{instr_id}"
                with h5py.File("../datasets/REVERIE/features/reverie_obj_features.hdf5", 'r') as f:
                    ft = f[key][...].astype(np.float32)
                target_fts.append(ft)
                target_mask[idx, 0] = True
            else:
                target_fts.append(np.zeros((1, self.args.image_feat_size), dtype=np.float32))
