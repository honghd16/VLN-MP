...
class GMapNavAgent(Seq2SeqAgent):
        ...
        for idx, ob in enumerate(obs):
            if ob['mp']:
                ...
                if self.args.full_obj:
                    filename = "../../transform_format/utsd_obj_full_features.hdf5"
                else:
                    filename = "../../transform_format/utsd_obj_features.hdf5"
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        language_inputs = self._prompt_variable(obs)
        ...
        for t in range(self.args.max_action_len):
            ...
            if train_ml is not None:
                ...
                elif self.args.dataset == 'cvdn':
                    nav_targets = self._teacher_action_r4r(
                        obs, nav_vpids, ended,
                        visited_masks=nav_inputs['gmap_visited_masks'] if self.args.fusion != 'local' else None,
                        imitation_learning=(self.feedback == 'teacher'), t=t, traj=traj
                    )
                elif self.args.dataset == 'utsd':
                    nav_targets = self._teacher_action_r4r(
                        obs, nav_vpids, ended,
                        visited_masks=nav_inputs['gmap_visited_masks'] if self.args.fusion != 'local' else None,
                        imitation_learning=(self.feedback == 'teacher'), t=t, traj=traj
