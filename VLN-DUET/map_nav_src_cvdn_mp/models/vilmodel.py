...
class MultimodalBertEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        self.visual_mlps = build_mlp(config.image_feat_size, hidden_dim=1024, 
                                        ...
                                        norm_type="layernorm")
        self.image_position_embeddings = nn.Embedding(1, config.hidden_size)

    def forward(self, input_ids, target_feats, bert_emb):
        seq_length = input_ids.size(1)
        ...
        image_position_ids = torch.arange(1, dtype=torch.long, device=input_ids.device)
        image_position_ids = image_position_ids.unsqueeze(0).expand(target_feats.size(0), -1)
        
        token_type_ids = torch.zeros((input_ids.size(0), seq_length+1), dtype=torch.long, device=input_ids.device)
        ...
        embeddings = prompt_embeddings + position_embeddings + token_type_embeddings
...
class MultimodalFusion_variant1(nn.Module):
    ...
    def forward(self, txt_embs, target_feats):        
...
class MultimodalFusion_variant3(nn.Module):
    ...
    def forward(self, txt_embs, target_feats, extended_txt_masks):        
        image_position_ids = torch.arange(1, dtype=torch.long, device=txt_embs.device)
        image_position_ids = image_position_ids.unsqueeze(0).expand(target_feats.size(0), -1)
        image_position_embeddings = self.image_position_embeddings(image_position_ids)

        target_embeddings = self.visual_mlps(target_feats)
        target_embs = target_embeddings + image_position_embeddings
        embeddings = torch.cat([txt_embs, target_embs], dim=1)
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    ...
    def forward_text(self, txt_ids, txt_masks, target_fts):
        txt_embeds = self.mm_embeddings(txt_ids, target_fts, self.embeddings)
        txt_embeds = self.lang_encoder(txt_embeds, txt_masks)

        # # for variant 1 & 2
        # txt_token_type_ids = torch.zeros_like(txt_ids)
        # original_txt_masks = txt_masks[:,:-1]
        ...
        # txt_embeds = self.fusion_layer(txt_embeds, target_fts)
        ...
        # txt_embeds = self.lang_encoder(txt_embeds, original_txt_masks)
        # extended_txt_masks = extend_neg_masks(txt_masks)
        # txt_embeds = self.fusion_layer(txt_embeds, target_fts, extended_txt_masks)
    ...
    def forward(self, mode, batch, **kwargs):
        if mode == 'language':
            txt_embeds = self.forward_text(batch['txt_ids'], batch['txt_masks'], batch['target_fts'])
            return txt_embeds
