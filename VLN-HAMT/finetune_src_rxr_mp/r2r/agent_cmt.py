...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        for split in ['train', 'val_seen', 'val_unseen']:
            ...
            json_path = os.path.join(args.anno_dir, 'new_landmark', f"rxr_landmarks_{split}_guide.jsonl")
        ...
        # Load landmark jsonl
        self.landmark_data_backup = {}
        for split in ['train', 'val_seen', 'val_unseen']:
            self.landmark_data_backup[split] = {}
            json_path = os.path.join(args.anno_dir, 'new_landmark', f"rxr_landmarks_{split}_guide.jsonl")
            with open(json_path, 'r') as f:
                for line in f:
                    obj = json.loads(line)
                    instr_id = obj['instruction_id']
                    self.landmark_data_backup[split][str(instr_id)] = obj
    ...
    def read_lm_fts(self, obs):
        ...
        for idx in ids:
            ...
            num_lm = len(instr_data["text_spans"])
            if num_lm == 0:
                instr_data = self.landmark_data_backup[split][instr_id]
                num_lm = len(instr_data["text_spans"])
            seq_lengths.append(num_lm)
        seq_tensor = torch.zeros((len(obs), max(seq_lengths), 512))
        for i, idx in enumerate(ids):
            instr_id = idx.split('_')[1]
            instr_data = self.landmark_data[split][instr_id]
            num_lm = len(instr_data["text_spans"])
            h5_file = "../datasets/RxR/features/nosim_openai_pth_vit_base_patch32_224_clip_300.hdf5"
            if num_lm == 0:
                instr_data = self.landmark_data_backup[split][instr_id]
                num_lm = len(instr_data["text_spans"])
                h5_file = "../datasets/RxR/features/nosim_openai_pth_vit_base_patch32_224_clip_300.hdf5"
            fts = []
            for ix in range(num_lm):
                if "idx" in instr_data.keys():
                    key = '%s_%s'%(instr_id, instr_data["idx"][ix])
                else:
                    key = '%s_%s'%(instr_id, ix)
                with h5py.File(h5_file, 'r') as f:
                    ft = f[key][...].astype(np.float32)
                    fts.append(ft)
            fts = torch.from_numpy(np.stack(fts, 1))
            seq_tensor[i, :num_lm] = fts
    ...
    def read_lm_pos(self, obs):
        ...
        for idx in ids:
            ...
            if len(lm_data["text_spans"]) == 0:
                lm_data = self.landmark_data_backup[split][instr_id]
            lm_locs = lm_data["landmark_locs"]
            lm_spans = lm_data["enc_spans"]
            num_before = 0
            final_pos = []
            for i in range(len(lm_locs)):
                final_pos.append(lm_locs[i]+lm_spans[i]+num_before)
                num_before += 1
            lm_pos.append(final_pos)
        return lm_pos

    def read_lm_fts_mask(self, obs, per):
        ids = [ob['instr_id'] for ob in obs]
        seq_lengths = []
        split = 'train' if self.env.name == 'val_train_seen' else self.env.name
        for idx in ids:
            ...
            if num_lm == 0:
                instr_data = self.landmark_data_backup[split][instr_id]
                num_lm = len(instr_data["text_spans"])
            masked_num_lm = int(np.ceil(num_lm * per))
            seq_lengths.append(masked_num_lm)
        seq_tensor = torch.zeros((len(obs), max(seq_lengths), 512))
        for i, idx in enumerate(ids):
            instr_id = idx.split('_')[1]
            instr_data = self.landmark_data[split][instr_id]
            ...
            masked_num_lm = int(np.ceil(num_lm * per))
            fts = []
            for ix in range(masked_num_lm):
                key = '%s_%s'%(instr_id, ix)
                with h5py.File(h5_file, 'r') as f:
                    ft = f[key][...].astype(np.float32)
                    fts.append(ft)
            fts = torch.from_numpy(np.stack(fts, 1))
            seq_tensor[i, :masked_num_lm] = fts
            
        return seq_tensor.cuda(), seq_lengths

    def read_lm_pos_mask(self, obs, per):
        ids = [ob['instr_id'] for ob in obs]
        split = 'train' if self.env.name == 'val_train_seen' else self.env.name
        lm_pos = []
        for idx in ids:
            ...
            lm_locs = lm_data["landmark_locs"]
            lm_spans = lm_data["enc_spans"]
            masked_num_lm = int(np.ceil(len(lm_locs) * per))
            num_before = 0
            final_pos = []
            for i in range(masked_num_lm):
                final_pos.append(lm_locs[i]+lm_spans[i]+num_before)
                num_before += 1
            lm_pos.append(final_pos)
        return lm_pos
    ...
    def _random_action(self, obs, ended):
        """
        Extract teacher actions into variable.
        :param obs: The observation.
        :param ended: Whether the action seq is ended
        ...
        for i, ob in enumerate(obs):
            if ended[i]:                                            # Just ignore this index
                ...
                a[i] = np.random.randint(len(ob['candidate'])+1)
        return torch.from_numpy(a).cuda()
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        :return:
        """
        if self.feedback == 'teacher' or self.feedback == 'argmax' or self.feedback == 'random':
            train_rl = False
        ...
        # # Landmark input
        # land_embeds, land_lens = self.read_lm_fts(obs)
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.feedback == 'teacher':
                ...
                a_t = self._random_action(obs, ended)
            else:
                print(self.feedback)
