...
class LandmarkEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.img_linear = nn.Linear(config.image_feat_size, config.hidden_size)
        self.img_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
    ...
    def forward(self, img_feat, type_embeddings):
        seq_length = img_feat.size(1)

        transformed_im = self.img_layer_norm(self.img_linear(img_feat))

        position_ids = torch.arange(seq_length, dtype=torch.long, device=img_feat.device)
        position_ids = position_ids.unsqueeze(0).expand_as(img_feat[:,:,0])
        position_embeddings = self.position_embeddings(position_ids)
...
class LandmarkEmbeddingsNoPos(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.img_linear = nn.Linear(config.image_feat_size, config.hidden_size)
        self.img_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
...
class MultimodalEmbeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.mm_linear = nn.Linear(config.hidden_size, config.hidden_size)
        self.mm_layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.position_embeddings = nn.Embedding(100 + config.max_position_embeddings, config.hidden_size)

        # tf naming convention for layer norm
        self.layer_norm = BertLayerNorm(config.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, txt_embeds, lm_embeds, lm_pos):
        if lm_embeds is not None:
            mm_embeds= torch.cat([txt_embeds, lm_embeds], dim=1)  
            lm_length = lm_embeds.size(1)
        else:
            mm_embeds = txt_embeds
            lm_length = None
        txt_length = txt_embeds.size(1)

        transformed_mm = self.mm_layer_norm(self.mm_linear(mm_embeds))

        if lm_length is not None:
            position_ids = self.generate_pos(txt_length, lm_length, lm_pos)
        else:
            position_ids = torch.arange(txt_length, dtype=torch.long, device="cuda")
            position_ids = position_ids.unsqueeze(0).repeat(8,1)

        position_embeddings = self.position_embeddings(position_ids)
        
        embeddings = transformed_mm + position_embeddings
        embeddings = self.layer_norm(embeddings)
        embeddings = self.dropout(embeddings)
        return embeddings
    
    def generate_pos(self, txt_length, lm_length, lm_pos):
        
        all_pos = []

        for p in lm_pos:
            t_pos = [x for x in range(txt_length + len(p)) if x not in p][:txt_length]
            max_t_pos = max(t_pos)
            l_pos = []
            num = 1
            for x in p:
                if x < max_t_pos:
                    ...
                    l_pos.append(max_t_pos+num)
                    num += 1
            r_pos = [txt_length + len(p)] * (lm_length - len(p))

            all_pos.append(t_pos + l_pos + r_pos)

        all_pos = torch.tensor(all_pos, dtype=torch.long, device="cuda")
        return all_pos
...
class NavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.lm_embeddings = LandmarkEmbeddingsNoPos(config)
        # self.lm_embeddings = LandmarkEmbeddings(config)
        self.mm_embeddings = MultimodalEmbeddings(config)
    ...
    def forward(self, mode, txt_ids=None, txt_embeds=None, txt_masks=None,
                land_embeds=None, land_masks=None, land_pos=None,
        ...
        if mode == 'language':
            ...
            if land_embeds is not None:
                # landmark embedding
                extended_lm_masks = land_masks.unsqueeze(1).unsqueeze(2)
                extended_lm_masks = extended_lm_masks.to(dtype=self.dtype)
                extended_lm_masks = (1.0 - extended_lm_masks) * -10000.0

                lm_token_type_ids = torch.ones(land_embeds.size(0), land_embeds.size(1), dtype=torch.long, device=self.device)
                lm_embeds = self.lm_embeddings(land_embeds, self.embeddings.token_type_embeddings(lm_token_type_ids))
            ...
            if land_embeds is not None:
                # txt_embeds = torch.cat([txt_embeds, lm_embeds], dim=1)
                txt_embeds = self.mm_embeddings(txt_embeds, lm_embeds, land_pos)
                extended_txt_masks = torch.cat([extended_txt_masks, extended_lm_masks], dim=-1)
            else:
                txt_embeds = self.mm_embeddings(txt_embeds, None, None)
