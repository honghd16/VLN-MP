...
def build_dataset(args, rank=0, is_test=False):
    ...
    train_env = dataset_class(
        feat_db, obj_db, train_instr_data, args.connectivity_dir, args.anno_dir, args.train_mp_prob,
    ...
    for split in val_env_names:
        ...
        val_env = dataset_class(
            feat_db, obj_db, val_instr_data, args.connectivity_dir, args.anno_dir, mp_prob=-1.0,
            batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
        ...
        val_instr_data_mp = construct_instrs(
            args.anno_dir, args.dataset, [split], 
            tokenizer=args.tokenizer, max_instr_len=args.max_instr_len
        )
        val_env_mp = dataset_class(
            feat_db, obj_db, val_instr_data_mp, args.connectivity_dir, args.anno_dir, mp_prob=2.0,
            batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_mp",
            multi_endpoints=False, max_objects=args.max_objects, multi_startpoints=False
        )
        val_envs[split+"_mp"] = val_env_mp
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    # # fix other parts
    # for name, param in listner.vln_bert.named_parameters():
    #     if "mm" in name:
    #         param.requires_grad = True
    #     else:
    #         param.requires_grad = False
    # for name, param in listner.critic.named_parameters():
    #     param.requires_grad = False
