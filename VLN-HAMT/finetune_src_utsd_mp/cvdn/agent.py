...
class NavCMTAgent(Seq2SeqCMTAgent):
    def _prompt_variable(self, obs):
        ...
        target_fts = []
        target_mask = torch.zeros((len(obs), 1), dtype=torch.bool)
        for idx, ob in enumerate(obs):
            if ob['mp']:
                key = f"{ob['scan']}_{ob['instr_id']}"
                if self.args.full_obj:
                    filename = "../datasets/UTSD/features/utsd_obj_full_features.hdf5"
                else:
                    filename = "../datasets/UTSD/features/utsd_obj_features.hdf5"
        ...
        target_fts = torch.from_numpy(target_fts)
        mask = torch.cat([mask, target_mask], 1)

        return seq_tensor.long().cuda(), mask.cuda(), target_fts.cuda()
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        # Language input
        txt_ids, txt_masks, target_fts = self._prompt_variable(obs)
