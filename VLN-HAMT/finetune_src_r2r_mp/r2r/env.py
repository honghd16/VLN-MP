...
class R2RBatch(object):
    ...
    def __init__(
        ...
        # load gelr2r
        if 'aug' not in self.name:
            type = 'train' if 'train' in self.name else self.name
            if mp_type in type:
                type = type[:-len(mp_type)-1]
            with open(f'../../VLN-DUET/datasets/R2R/annotations/GELR2R/{type}_gel.jsonl', 'r') as f:
                lines = f.readlines()
                self.gelr2r = {}
                for line in lines:
                    data = json.loads(line)
                    self.gelr2r[data['path_id']] = data
    ...
    def _get_obs(self, t=None, shortest_teacher=False):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            ob['distance'] = self.shortest_distances[state.scanId][state.location.viewpointId][item['path'][-1]]
            
            # decide visual prompts
            missing_path_ids = [2222, 2951, 601, 5476, 7053, 2847, 3108, 1404, 6939, 3090]
            if self.name != 'aug' and item['path_id'] not in missing_path_ids:
                lm_data = self.gelr2r[str(item['path_id'])]
                instr_id = int(item['instr_id'].split('_')[1])
                landmarks = lm_data["text_spans"][instr_id]
                if len(landmarks) == 0:
                    ob['landmarks'] = []
                    ob['mp_pos'] = []
                    ob['text_span'] = []
                else:
                    ob['landmarks'] = landmarks
                    spans = lm_data["text_tokenid_indices"][instr_id]
                    ob['text_span'] = []
                    for i,span in enumerate(spans):
                        last_one_index = len(span) - span[::-1].index(1) - 1
                        ob['text_span'].append(last_one_index)

                    if random.random() <= self.mp_prob:
                        if self.mp_type == 'one':
                            lm_idx = random.choice(range(len(landmarks)))
                            ob['mp_pos'] = [0]*len(landmarks)
                            ob['mp_pos'][lm_idx] = 1
                        elif self.mp_type == 'all':
                            ob['mp_pos'] = [1]*len(landmarks)
                        elif self.mp_type == 'random':
                            ob['mp_pos'] = [random.choice([0, 1]) for _ in range(len(landmarks))]
                        elif self.mp_type == 'random_num':
                            mp_num = random.choice(range(len(landmarks)))
                            ob['mp_pos'] = [0]*len(landmarks)
                            indices = random.sample(range(len(landmarks)), mp_num)
                            for idx in indices:
                                ob['mp_pos'][idx] = 1
                        elif self.mp_type == 'last':
                            ob['mp_pos'] = [0]*len(landmarks)
                            ob['mp_pos'][-1] = 1
                        else:
