...
def build_dataset(args, rank=0, is_test=False):
    ...
    train_env = dataset_class(
        ...
        mp_prob=args.train_mp_prob, mp_type=args.train_mp_type
    )
    ...
    for split in val_env_names:
        ...
        # one
        val_instr_data_one = construct_instrs(
            args.anno_dir, args.dataset, [split], tokenizer=tok, max_instr_len=args.max_instr_len
        )
        val_env_one = dataset_class(
            feat_db, val_instr_data_one, args.connectivity_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_one",
            mp_prob=2.0, mp_type='one'
        )
        val_envs[split+"_one"] = val_env_one

        # all
        val_instr_data_all = construct_instrs(
            args.anno_dir, args.dataset, [split], tokenizer=tok, max_instr_len=args.max_instr_len
        )
        val_env_all = dataset_class(
            feat_db, val_instr_data_all, args.connectivity_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_all",
            mp_prob=2.0, mp_type='all'
        )
        val_envs[split+"_all"] = val_env_all

        # random
        val_instr_data_random = construct_instrs(
            args.anno_dir, args.dataset, [split], tokenizer=tok, max_instr_len=args.max_instr_len
        )
        val_env_random = dataset_class(
            feat_db, val_instr_data_random, args.connectivity_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_random",
            mp_prob=2.0, mp_type='random'
        )
        val_envs[split+"_random"] = val_env_random

        # random_num
        val_instr_data_random_num = construct_instrs(
            args.anno_dir, args.dataset, [split], tokenizer=tok, max_instr_len=args.max_instr_len
        )
        val_env_random_num = dataset_class(
            feat_db, val_instr_data_random_num, args.connectivity_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_random_num",
            mp_prob=2.0, mp_type='random_num'
        )
        val_envs[split+"_random_num"] = val_env_random_num

        # last
        val_instr_data_last = construct_instrs(
            args.anno_dir, args.dataset, [split], tokenizer=tok, max_instr_len=args.max_instr_len
        )
        val_env_last = dataset_class(
            feat_db, val_instr_data_last, args.connectivity_dir, batch_size=args.batch_size, 
            angle_feat_size=args.angle_feat_size, seed=args.seed+rank,
            sel_data_idxs=None if args.world_size < 2 else (rank, args.world_size), name=split+"_last",
            mp_prob=2.0, mp_type='last'
        )
        val_envs[split+"_last"] = val_env_last
